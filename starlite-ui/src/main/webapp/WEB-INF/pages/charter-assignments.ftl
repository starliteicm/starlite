...
<html>
	<head>
  	...
  	<script type="text/javascript" src="${request.contextPath}/js/jquery.autocomplete.js"></script>
		...
		<script>
			var model = new ITAO.widget.roster.Model("services/schedule/charter/${charter.id}");
		
			YAHOO.util.Event.onContentReady("rosterDiv", function() {
				var d = new Date();
				<#if charter.startDatePlain??>
				d.setTime(Date.parse("${charter.startDatePlain?string('yyyy/MM/dd')}"));
				</#if>
	...
	<body>
		...
		<br/>
		<form id="addAllocationForm" action="${request.contextPath}/charter!addAllocation.action" method="POST">
			<!-- The charter's Id -->
			<input type="hidden" name="id" value="${id}"/>
			<h3>Assign Crew or Aircraft</h3>
			<label for="nameOrReg">Name/Reg</label>
			<input type="text" id="nameOrReg" name="nameOrReg"/>
			<br/>
			<label for="startDate">Start Date</label>
			<input type="text" name="startDate" class="date-pick" value="<#if charter.startDatePlain??>${charter.startDatePlain?string('dd/MM/yyyy')}</#if>"/>
			<br/>
			<label for="endDate">End Date</label>
			<input type="text" name="endDate" class="date-pick" value="<#if charter.endDatePlain??>${charter.endDatePlain?string('dd/MM/yyyy')}</#if>"/>
			<input type="submit" value="Add"/>
		</form>
		</div>
	<script>
		$(document).ready(function() {
			$("#nameOrReg").autocompleteArray(${assignableArray}, {delay:10, minChars:1, matchSubset:1, autoFill:true, maxItemsToShow:10, selectFirst:true});
		});
	</script>
	</body>
</html>
