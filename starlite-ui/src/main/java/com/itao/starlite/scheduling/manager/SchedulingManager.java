			...
			if (a.getAssignmentType().equals("Charter")) {
				for (Charter c: charters) {
					if (c.getId().equals(a.getAssignmentId())) {
						...
						ass.type = "Charter";
						ass.label = c.getCode();
						ass.from = c.getStartDatePlain();
						ass.to = c.getEndDatePlain();
...
//		a.label = c.getCode();
	...
	@Transactional
	public void saveAllocation(Allocation a) throws CausesCollisionsException {
		...
		if (causesCollisions)
			throw new CausesCollisionsException("Allocation causes collisions.");
