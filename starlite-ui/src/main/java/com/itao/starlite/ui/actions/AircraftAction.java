...
//		coaExpiry.getCellRenderer().setCellEditor(new DateOrBasicEditor());
	...
	private void prepareTabs() {
		...
		Tab aircraftFlightActualsTab = new Tab("OFP (To Approve)", "aircraft!flightOFPApproval.action", tab.equals("approve ofp"));
		...
		Tab aircraftFlightOFPTab = new Tab("OFP (New\\Admin)", "aircraft!flightOFP.action", tab.equals("flight OFP New"));
		Tab aircraftFlightOFPReadyTab = new Tab("OFP (Ready\\Realease)", "aircraft!flightOFPRelease.action", tab.equals("flight OFP Release"));
		Tab aircraftFlightOFPALLTab = new Tab("OFP (Completed)", "aircraft!flightOFPCompleted.action", tab.equals("flight OFP Completed"));
		
		/*if (user.hasPermission("ManagerView"))
			tableTabs = new Tab[] {aircraftTab, aircraftTypeTab,aircraftFlightOFPTab,aircraftFlightActualsTab,aircraftFlightPlansTab,aircraftFlightMisconfigurationsTab};
		*/
		if (user.hasPermission("ManagerView"))
			tableTabs = new Tab[] {aircraftTab, aircraftTypeTab,aircraftFlightOFPTab,aircraftFlightActualsTab,aircraftFlightOFPReadyTab,aircraftFlightOFPALLTab};
	...
	/*---------------------------------------------------------*/
	{
		...
		tableFacade.setColumnProperties("flightPlan_flightPlanID","flightActualStatus_flightActualStatusID","flightActualsID","flightActualsID");
	...
	/*---------------------------------------------------------*/
	/*---------------------------------------------------------*/
	public String flightOFP()
	/*---------------------------------------------------------*/
	{
		//Set the breadcrumbs for the current activity
		this.breadcrumbs = new Breadcrumb[]{
				new Breadcrumb("<a href='aircraft.action'>Aircraft</a>"),
				new Breadcrumb("<a href='aircraft!flightOFP.action'>OFP New</a>")};
		
		current="flight OFP New";
		tab="flight OFP New";
		prepareTabs();
		
		List<FlightOFP> flightOFPs = manager.findAllFlightOFPs();
		Collections.reverse(flightOFPs);
		
		List<FlightOFP> temp = new ArrayList<FlightOFP>();
		for (FlightOFP ofp : flightOFPs)
		{
			if (ofp.getFlightActualStatus().getFlightActualStatusID() < 2)
			{
				temp.add(ofp);
			}
		}
		TableFacade tableFacade = TableFacadeFactory.createTableFacade("FlightOFPNew", ServletActionContext.getRequest());
		tableFacade.setColumnProperties("id","id","OFPDate","contract","aircraft","invoiceNo","flightType","gCatagory","flightactualstatus_flightactualstatusid");
		tableFacade.setItems(temp);
		tableFacade.setMaxRows(15);
		
		Table table = tableFacade.getTable();
		table.getRow().setUniqueProperty("id");
		
		Column refCol1 = table.getRow().getColumn("id");
		refCol1.setTitle("ID");
	    refCol1.getCellRenderer().setCellEditor(new CellEditor() {
					public Object getValue(Object item, String property, int rowCount) {
						return ((FlightOFP) item).getId();
					}
			});
		
	    Column refCol5 = table.getRow().getColumn("OFPDate");
		refCol5.setTitle("Date");
        refCol5.getCellRenderer().setCellEditor(new CellEditor() {
					public Object getValue(Object item, String property, int rowCount) 
					{
						if (((FlightOFP) item).getOFPDate() == null)
						{
								return "";
						}
						return (""+((FlightOFP) item).getOFPDate());
					}
			});
        
		Column refCol2 = table.getRow().getColumn("contract");
		refCol2.setTitle("Contract");
        refCol2.getCellRenderer().setCellEditor(new CellEditor() {
					public Object getValue(Object item, String property, int rowCount) 
					{
						if (((FlightOFP) item).getContract() == null)
						{
								return "";
						}
						return (""+((FlightOFP) item).getContract());
					}
			});
        
        Column refCol3 = table.getRow().getColumn("aircraft");
		refCol3.setTitle("Aircraft");
		refCol3.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			   { if (""+((FlightOFP) item).getAircraft() == null)
			   {
				   return "";
			   }
				return (""+((FlightOFP) item).getAircraft().getRef());
				}
	    });
		
		Column refCol6 = table.getRow().getColumn("invoiceNo");
		refCol6.setTitle("Invoice No");
		refCol6.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			{
				if (((FlightOFP) item).getInvoiceNo() == null)
				{
						return "";
				}
				return (""+((FlightOFP) item).getInvoiceNo());
				}
	    });
		...
		refCol4.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			{
				if (((FlightOFP) item).getFlightType() == null)
				{
						return "";
				}
				return ((FlightOFP) item).getFlightType();
			}
	    });
		
		Column refCol7 = table.getRow().getColumn("gCatagory");
		refCol7.setTitle("G-Category");
		refCol7.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			{
				if (((FlightOFP) item).getGCatagory() == null)
				{
						return "";
				}
				return ((FlightOFP) item).getGCatagory().getValue();
			}
	    });
		
		
		Column refCol8 = table.getRow().getColumn("id");
		refCol8.setTitle("ID");
		refCol8.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			{
				return ((FlightOFP) item).getId();
			}
		...
		Column refCol9 = table.getRow().getColumn("flightactualstatus_flightactualstatusid");
		refCol9.setTitle("Status");
		refCol9.getCellRenderer().setCellEditor(new CellEditor() {
			public Object getValue(Object item, String property, int rowCount) 
			{
				if (((FlightOFP) item).getFlightActualStatus() == null)
				{
						return "";
				}
				return ((FlightOFP) item).getFlightActualStatus().getFlightActualStatusValue();
			}
	    });
		...
		refCol.getCellRenderer().setCellEditor(new CellEditor() {
			...
			public Object getValue(Object item, String property, int rowCount) {
				Object value = new BasicCellEditor().getValue(item, property, rowCount);
				HtmlBuilder html = new HtmlBuilder();
				html.a().href().quote().append("flightOFP!edit.action?id="+value+"&edited=true").quote().close();
		...
		tableFacade.setView(new SearchTableView());
		tableHtml = tableFacade.render();
		return "flight OFP New";
	}//flightPlans()
	/*---------------------------------------------------------*/
	public String flightOFPApproval()
	/*---------------------------------------------------------*/
	{
	//Set the breadcrumbs for the current activity
	this.breadcrumbs = new Breadcrumb[]{
			new Breadcrumb("<a href='aircraft.action'>Aircraft</a>"),
			new Breadcrumb("<a href='aircraft!flightOFP.action'>OFP Approval</a>")};
	
	current="flight OFP Approval";
	tab="flight OFP Approval";
	prepareTabs();
	
	List<FlightOFP> flightOFPs = manager.findAllFlightOFPs();
	Collections.reverse(flightOFPs);
	//List<FlightOFP> toApprove = new ArrayList<FlightOFP>();
	
	List<FlightOFP> temp = new ArrayList<FlightOFP>();
	for (FlightOFP ofp : flightOFPs)
	{
		if ((ofp.getFlightActualStatus().getFlightActualStatusID() > 1)&&(ofp.getFlightActualStatus().getFlightActualStatusID() < 4))
		{
			temp.add(ofp);
		}
	}
	final String params = "id="+this.typeId;
	
	TableFacade tableFacade = TableFacadeFactory.createTableFacade("FlightOFPApproval", ServletActionContext.getRequest());
	tableFacade.setColumnProperties("id","OFPDate","contract","aircraft","invoiceNo","flightType","gCatagory","flightactualstatus_flightactualstatusid");
	tableFacade.setItems(temp);
	tableFacade.setMaxRows(15);
	...
	refCol.getCellRenderer().setCellEditor(new CellEditor() {
		...
		public Object getValue(Object item, String property, int rowCount) {
			...
			html.a().href().quote().append("flightOFP!edit.action?id="+value+"&edited=true").quote().close();
			//html.a().href().quote().append("flightActualsApproval!addType.action?typeID="+value).quote().close();
			html.append(value+"&nbsp;&nbsp;&nbsp;View");
			html.aEnd();
			
			HtmlBuilder html2 = new HtmlBuilder();
			html2 = new HtmlBuilder();
			html2.a().href().quote().append("flightOFP!approval.action?id="+value+"&edited=true&approval=1&typeId=4").quote().close();
			//html.a().href().quote().append("flightActualsApproval!addType.action?typeID="+value).quote().close();
			html2.append("Approve");
			html2.aEnd();
	...
	tableFacade.setView(new SearchTableView());
	tableHtml = tableFacade.render();
	return "approve ofp Approval";
}//flightPlans()
/*---------------------------------------------------------*/
	public String flightOFPRelease()
	/*---------------------------------------------------------*/
	{
	//Set the breadcrumbs for the current activity
	this.breadcrumbs = new Breadcrumb[]{
			new Breadcrumb("<a href='aircraft.action'>Aircraft</a>"),
			new Breadcrumb("<a href='aircraft!flightOFP.action'>OFP Renew\\Release</a>")};
	
	current="flight OFP Release";
	...
	for (FlightOFP ofp : flightOFPs)
	{
		if ((ofp.getFlightActualStatus().getFlightActualStatusID() > 3)&&(ofp.getFlightActualStatus().getFlightActualStatusID() < 7))
		{
			temp.add(ofp);
		}
	}
	TableFacade tableFacade = TableFacadeFactory.createTableFacade("FlightOFPRelease", ServletActionContext.getRequest());
	tableFacade.setColumnProperties("id","OFPDate","contract","aircraft","invoiceNo","flightType","gCatagory","flightactualstatus_flightactualstatusid");
	tableFacade.setItems(temp);
	tableFacade.setMaxRows(15);
	...
	refCol.getCellRenderer().setCellEditor(new CellEditor() {
		...
		public Object getValue(Object item, String property, int rowCount) {
			...
			HtmlBuilder html2 = new HtmlBuilder();
			html2 = new HtmlBuilder();
			html2.a().href().quote().append("flightOFP!approval.action?id="+value+"&edited=true&approval=1&typeId=7").quote().close();
			//html.a().href().quote().append("flightActualsApproval!addType.action?typeID="+value).quote().close();
			html2.append("Ready\\Release");
			html2.aEnd();
			return html.toString() + "&nbsp;&nbsp;&nbsp;"+html2.toString();
		}
	...
	tableFacade.setView(new SearchTableView());
	tableHtml = tableFacade.render();
	return "flight OFP Release";
}//flightPlans()
/*---------------------------------------------------------*/
	public String flightOFPCompleted()
	/*---------------------------------------------------------*/
	{
	//Set the breadcrumbs for the current activity
	this.breadcrumbs = new Breadcrumb[]{
			new Breadcrumb("<a href='aircraft.action'>Aircraft</a>"),
			new Breadcrumb("<a href='aircraft!flightOFP.action'>OFPs Completed</a>")};
	
	current="flight OFP Completed";
	tab="flight OFP Completed";
	prepareTabs();
	...
	for (FlightOFP ofp : flightOFPs)
	{
		if ((ofp.getFlightActualStatus().getFlightActualStatusID() > 6))
		{
			temp.add(ofp);
		}
	}
	TableFacade tableFacade = TableFacadeFactory.createTableFacade("FlightOFPCompleted", ServletActionContext.getRequest());
	tableFacade.setColumnProperties("id","OFPDate","contract","aircraft","invoiceNo","flightType","gCatagory","flightactualstatus_flightactualstatusid");
	tableFacade.setItems(temp);
	tableFacade.setMaxRows(15);
...
/*---------------------------------------------------------*/
	...
	/*---------------------------------------------------------*/
	{
		...
		Collections.reverse(flightPlans);
