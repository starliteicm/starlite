...
public class FlightActualStatusHibernateDao extends GenericHibernateDao<FlightActualStatus, Integer> implements FlightActualStatusDao {
	...
	public FlightActualStatus findStatusIDByValue(String value) {
       FlightActualStatus tempActual = null;
		...
		List<FlightActualStatus> tempList = (List<FlightActualStatus>)getCurrentSession().createQuery("from FlightActualStatus where flightActualStatusValue = ?").setParameter(0, value).list();
